...
class LabelClassProcessor:
    ...
    def process_identity(self, target_fc_path):
        ...
        try:
            ...
            target_name = os.path.basename(target_fc_path)
            output_name = target_name.replace("_контур", "_сетка")
            output_path = os.path.join(os.path.dirname(target_fc_path), output_name)
    ...
    def process_fields(self, output_path):
        ...
        try:
            ...
            # Очищаем значения в поле NPP на основе значений LandCode
            try:
                ...
                # Проверяем наличие поля LandCode
                if "LandCode" in [f.name for f in arcpy.ListFields(output_path)]:
                    logging.info("Начало очистки поля NPP на основе значений LandCode")
                    
                    # Разрешенные значения LandCode, при которых NPP не очищается
                    allowed_codes = [123, 3, 6, 7]
                    logging.info("Разрешенные значения LandCode: {}".format(", ".join(map(str, allowed_codes))))
                    
                    # Создаем курсор обновления с полями NPP и LandCode
                    cleared_count = 0
                    with arcpy.da.UpdateCursor(output_path, [npp_field_name, "LandCode"]) as cursor:
                        for row in cursor:
                            npp_value = row[0]
                            land_code = row[1]
                            
                            # Если LandCode не входит в список разрешенных, очищаем NPP
                            if land_code is not None and land_code not in allowed_codes:
                                row[0] = None  # Устанавливаем NPP в NULL
                                cursor.updateRow(row)
                                cleared_count += 1
                    
                    logging.info("Очищено {} значений в поле NPP, где LandCode не входит в список разрешенных".format(cleared_count))
                else:
                    logging.warning("Поле LandCode не найдено, очистка NPP не выполнена")
            except Exception as clear_err:
                logging.error("Ошибка при очистке поля NPP на основе LandCode: {}".format(str(clear_err)))
                messagebox.showwarning("Предупреждение", "Не удалось выполнить очистку поля NPP на основе LandCode: {}".format(str(clear_err)))
                        ...
                        try:
                            ...
                            try:
                                ...
                                if not land_path:
                                    ...
                                    # Формируем имя для нового класса объектов
                                    land_clip_name = "Land_{}_контур".format(self.shortened_name)
                                    ...
                                    if not arcpy.Exists(land_clip_path):
                                        ...
                                        logging.info("Вырезание данных из '{}' по '{}', сохранение в '{}'".format(
                                            land_path, contour_fc_path, land_clip_path))
                                        
                                        # Используем инструмент Clip
                                        arcpy.Clip_analysis(
                                            land_path,  # Входной класс
                                            contour_fc_path,  # Вырезающий класс
                                        ...
                                        # 7. Удаляем из Land_"Сокр"_контур объекты с определенными кодами, 
                                        # которые не входят в границы Lots_"Сокр"
                                        try:
                                            logging.info("Начало удаления объектов с определенными кодами...")
                                            
                                            # Список кодов для проверки
                                            land_codes_to_check = [121, 122, 123, 131, 132, 3]
                                            logging.info("Коды для проверки: {}".format(land_codes_to_check))
                                            
                                            # Формируем SQL-выражение для выбора объектов с указанными кодами
                                            code_sql = "LandCode IN ({})".format(",".join(map(str, land_codes_to_check)))
                                            logging.info("SQL-выражение для выбора: {}".format(code_sql))
                                            
                                            # Создаем временный слой из Land_"Сокр"_контур
                                            temp_layer_name = "temp_land_layer"
                                            arcpy.MakeFeatureLayer_management(land_clip_path, temp_layer_name, code_sql)
                                            
                                            # Проверяем, сколько объектов выбрано
                                            selected_count = int(arcpy.GetCount_management(temp_layer_name).getOutput(0))
                                            logging.info("Выбрано объектов с указанными кодами: {}".format(selected_count))
                                            
                                            if selected_count > 0:
                                                # Выбираем из них те, которые пересекаются с Lots_"Сокр"
                                                ...
                                                # Инвертируем выборку, чтобы получить те, которые НЕ пересекаются
                                                arcpy.SelectLayerByAttribute_management(
                                                    temp_layer_name,
                                                    "SWITCH_SELECTION"
                                                )
                                                
                                                # Проверяем, сколько объектов осталось в выборке для удаления
                                                to_delete_count = int(arcpy.GetCount_management(temp_layer_name).getOutput(0))
                                                logging.info("Объектов с указанными кодами, не входящих в границы Lots_'{}': {}".format(
                                                    self.shortened_name, to_delete_count))
                                                
                                                if to_delete_count > 0:
                                                    # Удаляем объекты
                                                    arcpy.DeleteFeatures_management(temp_layer_name)
                                                    logging.info("Удалено объектов: {}".format(to_delete_count))
                                                else:
                                                    logging.info("Нет объектов для удаления")
                                            
                                            # Удаляем временный слой
                                            arcpy.Delete_management(temp_layer_name)
                                            logging.info("Временный слой удален")
                                            
                                            # Обновляем количество объектов после удаления
                                            final_count = int(arcpy.GetCount_management(land_clip_path).getOutput(0))
                                            logging.info("Количество объектов в '{}' после удаления: {}".format(
                                                land_clip_name, final_count))
                                            
                                            # Сохраняем информацию об удаленных объектах
                                            self.deleted_features_count = to_delete_count if 'to_delete_count' in locals() else 0
                                            
                                            # Дополнительно удаляем из Land_"Сокр"_контур объекты с кодом 326
                                            try:
                                                logging.info("Начало удаления объектов с кодом 326...")
                                                
                                                # Формируем SQL-выражение для выбора объектов с кодом 326
                                                code_326_sql = "LandCode = 326"
                                                logging.info("SQL-выражение для выбора: {}".format(code_326_sql))
                                                
                                                # Создаем временный слой из Land_"Сокр"_контур
                                                temp_layer_326 = "temp_land_layer_326"
                                                arcpy.MakeFeatureLayer_management(land_clip_path, temp_layer_326, code_326_sql)
                                                
                                                # Проверяем, сколько объектов выбрано
                                                selected_326_count = int(arcpy.GetCount_management(temp_layer_326).getOutput(0))
                                                logging.info("Выбрано объектов с кодом 326: {}".format(selected_326_count))
                                                
                                                if selected_326_count > 0:
                                                    # Удаляем объекты
                                                    arcpy.DeleteFeatures_management(temp_layer_326)
                                                    logging.info("Удалено объектов с кодом 326: {}".format(selected_326_count))
                                                    
                                                    # Обновляем общее количество удаленных объектов
                                                    if hasattr(self, 'deleted_features_count'):
                                                        self.deleted_features_count += selected_326_count
                                                    else:
                                                        self.deleted_features_count = selected_326_count
                                                else:
                                                    logging.info("Объектов с кодом 326 не найдено")
                                                
                                                # Удаляем временный слой
                                                arcpy.Delete_management(temp_layer_326)
                                                logging.info("Временный слой удален")
                                                
                                                # Обновляем количество объектов после удаления кода 326
                                                final_count_326 = int(arcpy.GetCount_management(land_clip_path).getOutput(0))
                                                logging.info("Количество объектов в '{}' после удаления кода 326: {}".format(
                                                    land_clip_name, final_count_326))
                                                
                                            except Exception as delete_326_err:
                                                logging.error("Ошибка при удалении объектов с кодом 326: {}".format(str(delete_326_err)))
                                                logging.error(traceback.format_exc())
                                                messagebox.showwarning(
                                                    "Предупреждение", 
                                                    "Ошибка при удалении объектов с кодом 326:\n{}".format(str(delete_326_err))
                                                )
                                            
                                            # 8. Вырезаем данные из класса Admi по границам контурного слоя
                                            try:
                                                ...
                                                if not admi_path:
                                                    ...
                                                    if not arcpy.Exists(admi_clip_path):
                                                        # Вырезаем данные из Admi по контуру
                                                        logging.info("Вырезание данных из '{}' по '{}', сохранение в '{}'".format(
                                            ...
                                            logging.error("Ошибка при удалении объектов: {}".format(str(delete_err)))
                                            logging.error(traceback.format_exc())
                                            messagebox.showwarning(
                                                "Предупреждение", 
                                                "Ошибка при удалении объектов:\n{}".format(str(delete_err))
                                            )
...
def main():
    ...
    # Формируем путь к контурному слою
    try:
        # Находим класс объектов для идентичности (Land_"Сокр"_контур)
        land_contour_name = "Land_{}_контур".format(shortened_name)
        ...
        # Полный путь к слою контура
        land_contour_path = os.path.join(dataset_path, land_contour_name)
        
        if not arcpy.Exists(land_contour_path):
            logging.error("Не найден класс объектов '{}'".format(land_contour_name))
            messagebox.showerror("Ошибка", "Не найден класс объектов '{}'".format(land_contour_name))
            return
        
        # Выполняем операцию идентичности
        success = label_processor.process_identity(land_contour_path)
